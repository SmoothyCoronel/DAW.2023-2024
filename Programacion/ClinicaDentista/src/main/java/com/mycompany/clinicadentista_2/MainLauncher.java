...
public class MainLauncher {
	...
	private static void altaPaciente(Scanner sc) {
		...
		tfno = leerStringTeclado(sc);
                ...
                Paciente p = new Paciente(nombre, apellidos, dni, fNac, tfno, email);
                
		// Crear paciente
		if (miClinica.nuevoPaciente(p)) {
	...
	private static void ponerTratamiento(Paciente p, Scanner sc) {
		...
		precio = leerFloatTeclado(sc);
                ...
                Tratamiento t = new Tratamiento(codigo, descripcion, fecha, precio);
                
		// Crear tratamiento
		if (p.nuevoTratamiento(t)) {
	...
	private static void saveClinica(int contador, Scanner sc) {
            /*
             * Aquí hay que poner llamada a Controlador de Ficheros
             * */
            
            //Pedir datos
            String fileName = "Clinica.txt";
            if (miClinica != null){
                Gson gson = new Gson();
                String clinicaDatos = gson.toJson(miClinica);     

                //Crear archivo y enviar los datos de Clinica a este                
                ControladorFicheros.writeText(fileName, clinicaDatos, true);
            }		
	}

	private static void loadClinica() {
            /*
             * Aquí hay que poner llamada a Controlador de Ficheros
             * */
